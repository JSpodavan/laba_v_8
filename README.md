...
## Задание :  Отдел кадров

Имеются сотрудники предприятия (табельный номер, ФИО, паспортные
данные, адрес, дата рождения, пол, должность и количество детей), их дети
(имя, пол, дата рождения) и отделы (наименование, количество закрепленных
помещений). Каждый работник работает в одном отделе в каждый
промежуток времени. Необходимо регистрировать хронологию назначений -
перемещений сотрудников (т.е. фиксировать дату начала работы сотрудника
в отделе и дату окончания).
...
В рамках лабораторной работы были созданы функции генерации случайных данных и выполнено наполнение таблиц
